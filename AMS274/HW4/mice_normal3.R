...
pi2 <- predict(mod2,newdata=list(conc=newconc),type="response") * 
  (1 - predict(mod1,newdata=list(conc=newconc),type="response"))
ppfit <- predict(mod2,newdata=list(conc=newconc),se.fit = T)$fit*
  (1 - predict(mod1,newdata=list(conc=newconc),se.fit = T)$fit)
...
lines(newconc, pi2, col='blue', lwd=1)
lines(newconc, linkinv1(pred0+sc*pp$se.fit), col = 'blue', lwd = 1, lty = 6)
lines(newconc, linkinv1(pred0-sc*pp$se.fit), col = 'blue', lwd = 1, lty = 6)
